...
struct ImGuiColorRLMod;               // Stacked color modifier, backup of modified data so we can restore it
...
struct ImGuiStyleRLMod;               // Stacked style modifier, backup of modified data so we can restore it
...
static inline int    ImRLModPositive(int a, int b)                                { return (a + b) % b; }
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_NoKeyRLModifiers         = 1 << 16,  // disable mouse interaction if a key modifier is held
...
struct ImGuiStyleRLMod
{
    ImGuiStyleVar   VarIdx;
    union           { int BackupInt[2]; float BackupFloat[2]; };
    ImGuiStyleRLMod(ImGuiStyleVar idx, int v)     { VarIdx = idx; BackupInt[0] = v; }
    ImGuiStyleRLMod(ImGuiStyleVar idx, float v)   { VarIdx = idx; BackupFloat[0] = v; }
    ImGuiStyleRLMod(ImGuiStyleVar idx, ImVec2 v)  { VarIdx = idx; BackupFloat[0] = v.x; BackupFloat[1] = v.y; }
};
...
#define ImGuiKey_NavKeyboardTweakSlow   ImGuiRLMod_Ctrl
#define ImGuiKey_NavKeyboardTweakFast   ImGuiRLMod_Shift
...
struct ImGuiKeyRoutingData
{
    ...
    ImU16                           RLMods;               // Technically we'd only need 4-bits but for simplify we store ImGuiRLMod_ values which need 16-bits. ImGuiRLMod_Shortcut is already translated to Ctrl/Super.
    ImU8                            RoutingNextScore;   // Lower is better (0: perfect score)
    ImGuiID                         RoutingCurr;
    ImGuiID                         RoutingNext;

    ImGuiKeyRoutingData()           { NextEntryIndex = -1; RLMods = 0; RoutingNextScore = 255; RoutingCurr = RoutingNext = ImGuiKeyOwner_None; }
};
...
struct ImGuiContext
{
    ...
    // - Routing is requested ahead of time for a given chord (Key + RLMods) and granted in NewFrame().
    ...
    // Shared stacks
    ImVector<ImGuiColorRLMod> ColorStack;                         // Stack for PushStyleColor()/PopStyleColor() - inherited by Begin()
    ImVector<ImGuiStyleRLMod> StyleVarStack;                      // Stack for PushStyleVar()/PopStyleVar() - inherited by Begin()
    ...
    ImGuiKeyChord           NavJustMovedToKeyRLMods;
    ...
    ImGuiKeyChord           NavMoveKeyRLMods;
    ...
    ImGuiKeyChord           ConfigNavWindowingKeyNext;          // = ImGuiRLMod_Ctrl | ImGuiKey_Tab, for reconfiguration (see #4828)
    ImGuiKeyChord           ConfigNavWindowingKeyPrev;          // = ImGuiRLMod_Ctrl | ImGuiRLMod_Shift | ImGuiKey_Tab
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas)
    {
        ...
        NavJustMovedToKeyRLMods = ImGuiRLMod_None;
        ...
        NavMoveKeyRLMods = ImGuiRLMod_None;
        NavMoveDir = NavMoveDirForDebug = NavMoveClipDir = ImGuiDir_None;
        NavScoringDebugCount = 0;
        NavTabbingDir = 0;
        NavTabbingCounter = 0;

        ConfigNavWindowingKeyNext = ImGuiRLMod_Ctrl | ImGuiKey_Tab;
        ConfigNavWindowingKeyPrev = ImGuiRLMod_Ctrl | ImGuiRLMod_Shift | ImGuiKey_Tab;
...
//#endif
    ...
    // Popups, RLModals, Tooltips
    IMGUI_API bool          BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);
    IMGUI_API void          OpenPopupEx(ImGuiID id, ImGuiPopupFlags popup_flags = ImGuiPopupFlags_None);
    IMGUI_API void          ClosePopupToLevel(int remaining, bool restore_focus_to_window_under_popup);
    IMGUI_API void          ClosePopupsOverWindow(ImGuiWindow* ref_window, bool restore_focus_to_window_under_popup);
    IMGUI_API void          ClosePopupsExceptRLModals();
    IMGUI_API bool          IsPopupOpen(ImGuiID id, ImGuiPopupFlags popup_flags);
    IMGUI_API bool          BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags);
    IMGUI_API bool          BeginTooltipEx(ImGuiTooltipFlags tooltip_flags, ImGuiWindowFlags extra_window_flags);
    IMGUI_API ImRect        GetPopupAllowedExtentRect(ImGuiWindow* window);
    IMGUI_API ImGuiWindow*  GetTopMostPopupRLModal();
    IMGUI_API ImGuiWindow*  GetTopMostAndVisiblePopupRLModal();
    ...
    inline bool             IsNamedKeyOrRLModKey(ImGuiKey key)                            { return (key >= ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END) || key == ImGuiRLMod_Ctrl || key == ImGuiRLMod_Shift || key == ImGuiRLMod_Alt || key == ImGuiRLMod_Super || key == ImGuiRLMod_Shortcut; }
    inline bool             IsLegacyKey(ImGuiKey key)                                   { return key >= ImGuiKey_LegacyNativeKey_BEGIN && key < ImGuiKey_LegacyNativeKey_END; }
    inline bool             IsKeyboardKey(ImGuiKey key)                                 { return key >= ImGuiKey_Keyboard_BEGIN && key < ImGuiKey_Keyboard_END; }
    inline bool             IsGamepadKey(ImGuiKey key)                                  { return key >= ImGuiKey_Gamepad_BEGIN && key < ImGuiKey_Gamepad_END; }
    inline bool             IsMouseKey(ImGuiKey key)                                    { return key >= ImGuiKey_Mouse_BEGIN && key < ImGuiKey_Mouse_END; }
    inline bool             IsAliasKey(ImGuiKey key)                                    { return key >= ImGuiKey_Aliases_BEGIN && key < ImGuiKey_Aliases_END; }
    inline ImGuiKeyChord    ConvertShortcutRLMod(ImGuiKeyChord key_chord)                 { ImGuiContext& g = *GImGui; IM_ASSERT_PARANOID(key_chord & ImGuiRLMod_Shortcut); return (key_chord & ~ImGuiRLMod_Shortcut) | (g.IO.ConfigMacOSXBehaviors ? ImGuiRLMod_Super : ImGuiRLMod_Ctrl); }
    inline ImGuiKey         ConvertSingleRLModFlagToKey(ImGuiContext* ctx, ImGuiKey key)
    {
        ImGuiContext& g = *ctx;
        if (key == ImGuiRLMod_Ctrl) return ImGuiKey_ReservedForRLModCtrl;
        if (key == ImGuiRLMod_Shift) return ImGuiKey_ReservedForRLModShift;
        if (key == ImGuiRLMod_Alt) return ImGuiKey_ReservedForRLModAlt;
        if (key == ImGuiRLMod_Super) return ImGuiKey_ReservedForRLModSuper;
        if (key == ImGuiRLMod_Shortcut) return (g.IO.ConfigMacOSXBehaviors ? ImGuiKey_ReservedForRLModSuper : ImGuiKey_ReservedForRLModCtrl);
        return key;
    }
    ...
    inline ImGuiKeyOwnerData*   GetKeyOwnerData(ImGuiContext* ctx, ImGuiKey key)                    { if (key & ImGuiRLMod_Mask_) key = ConvertSingleRLModFlagToKey(ctx, key); IM_ASSERT(IsNamedKey(key)); return &ctx->KeysOwnerData[key - ImGuiKey_NamedKey_BEGIN]; }
    ...
    // - ImGuiKeyChord = a ImGuiKey optionally OR-red with ImGuiRLMod_Alt/ImGuiRLMod_Ctrl/ImGuiRLMod_Shift/ImGuiRLMod_Super.
    //     ImGuiKey_C                 (accepted by functions taking ImGuiKey or ImGuiKeyChord)
    //     ImGuiKey_C | ImGuiRLMod_Ctrl (accepted by functions taking ImGuiKeyChord)
    //   ONLY ImGuiRLMod_XXX values are legal to 'OR' with an ImGuiKey. You CANNOT 'OR' two ImGuiKey values.
