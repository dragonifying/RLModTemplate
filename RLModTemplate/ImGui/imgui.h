...
enum ImGuiKey : int;                // -> enum ImGuiKey              // Enum: A key identifier (ImGuiKey_XXX or ImGuiRLMod_XXX value)
...
typedef int ImGuiKeyChord;          // -> ImGuiKey | ImGuiRLMod_XXX    // Flags: for storage only for now: an ImGuiKey optionally OR-ed with one or more ImGuiRLMod_XXX values.
...
namespace ImGui
{
    ...
    //  - BeginPopupRLModal(): block every interaction behind the window, cannot be closed by user, add a dimming background, has a title bar.
    IMGUI_API bool          BeginPopup(const char* str_id, ImGuiWindowFlags flags = 0);                         // return true if the popup is open, and you can start outputting to it.
    IMGUI_API bool          BeginPopupRLModal(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0); // return true if the modal is open, and you can start outputting to it.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_RLModal                  = 1 << 27,  // Don't use! For internal use by BeginPopupRLModal()
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackCharFilter  = 1 << 9,   // Callback on character inputs to replace or discard them. RLModify 'EventChar' to replace or discard, or return 1 in callback to discard.
    ...
    // Obsolete names
    //ImGuiInputTextFlags_AlwaysInsertRLMode  = ImGuiInputTextFlags_AlwaysOverwrite   // [renamed in 1.82] name was not matching behavior
};
...
enum ImGuiHoveredFlags_
{
    ...
    //ImGuiHoveredFlags_AllowWhenBlockedByRLModal     = 1 << 6,   // Return true even if a modal popup window is normally blocking access to this item/window. FIXME-TODO: Unavailable yet.
...
// A key identifier (ImGuiKey_XXX or ImGuiRLMod_XXX value): can represent Keyboard, Mouse and Gamepad values.
...
enum ImGuiKey : int
{
    ...
    ImGuiKey_ReservedForRLModCtrl, ImGuiKey_ReservedForRLModShift, ImGuiKey_ReservedForRLModAlt, ImGuiKey_ReservedForRLModSuper,
    ImGuiKey_COUNT,

    // Keyboard RLModifiers (explicitly submitted by backend via AddKeyEvent() calls)
    // - This is mirroring the data also written to io.KeyCtrl, io.KeyShift, io.KeyAlt, io.KeySuper, in a format allowing
    //   them to be accessed via standard key API, allowing calls such as IsKeyPressed(), IsKeyReleased(), querying duration etc.
    // - Code polling every key (e.g. an interface to detect a key press for input mapping) might want to ignore those
    //   and prefer using the real keys (e.g. ImGuiKey_LeftCtrl, ImGuiKey_RightCtrl instead of ImGuiRLMod_Ctrl).
    // - In theory the value of keyboard modifiers should be roughly equivalent to a logical or of the equivalent left/right keys.
    //   In practice: it's complicated; mods are often provided from different sources. Keyboard layout, IME, sticky keys and
    //   backends tend to interfere and break that equivalence. The safer decision is to relay that ambiguity down to the end-user...
    ImGuiRLMod_None                   = 0,
    ImGuiRLMod_Ctrl                   = 1 << 12, // Ctrl
    ImGuiRLMod_Shift                  = 1 << 13, // Shift
    ImGuiRLMod_Alt                    = 1 << 14, // Option/Menu
    ImGuiRLMod_Super                  = 1 << 15, // Cmd/Super/Windows
    ImGuiRLMod_Shortcut               = 1 << 11, // Alias for Ctrl (non-macOS) _or_ Super (macOS).
    ImGuiRLMod_Mask_                  = 0xF800,  // 5-bits
    ...
    ImGuiKey_RLModCtrl = ImGuiRLMod_Ctrl, ImGuiKey_RLModShift = ImGuiRLMod_Shift, ImGuiKey_RLModAlt = ImGuiRLMod_Alt, ImGuiKey_RLModSuper = ImGuiRLMod_Super, // Renamed in 1.89
...
enum ImGuiCol_
{
    ...
    ImGuiCol_RLModalWindowDimBg,      // Darken/colorize entire screen behind a modal window, when one is active
    ImGuiCol_COUNT
};
...
struct ImGuiIO
{
    ...
    ImGuiKeyChord KeyRLMods;                          // Key mods flags (any of ImGuiRLMod_Ctrl/ImGuiRLMod_Shift/ImGuiRLMod_Alt/ImGuiRLMod_Super flags, same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags. DOES NOT CONTAINS ImGuiRLMod_Shortcut which is pretranslated). Read-only, updated by NewFrame()
...
// - ImGuiInputTextFlags_CallbackCharFilter:  Callback on character inputs to replace or discard them. RLModify 'EventChar' to replace or discard, or return 1 in callback to discard.
...
struct ImFontConfig
{
    ...
    bool            MergeRLMode;              // false    // Merge into previous ImFont, so you can combine multiple inputs font into one ImFont (e.g. ASCII font + icons + Japanese glyphs). You may want to use GlyphOffset.y when merge font of different heights.
...
// RENAMED and MERGED both ImGuiKey_RLModXXX and ImGuiRLModFlags_XXX into ImGuiRLMod_XXX (from September 2022)
// RENAMED ImGuiKeyRLModFlags -> ImGuiRLModFlags in 1.88 (from April 2022). Exceptionally commented out ahead of obscolescence schedule to reduce confusion and because they were not meant to be used in the first place.
typedef ImGuiKeyChord ImGuiRLModFlags;      // == int. We generally use ImGuiKeyChord to mean "a ImGuiKey or-ed with any number of ImGuiRLMod_XXX value", but you may store only mods in there.
enum ImGuiRLModFlags_ { ImGuiRLModFlags_None = 0, ImGuiRLModFlags_Ctrl = ImGuiRLMod_Ctrl, ImGuiRLModFlags_Shift = ImGuiRLMod_Shift, ImGuiRLModFlags_Alt = ImGuiRLMod_Alt, ImGuiRLModFlags_Super = ImGuiRLMod_Super };
//typedef ImGuiKeyChord ImGuiKeyRLModFlags; // == int
//enum ImGuiKeyRLModFlags_ { ImGuiKeyRLModFlags_None = 0, ImGuiKeyRLModFlags_Ctrl = ImGuiRLMod_Ctrl, ImGuiKeyRLModFlags_Shift = ImGuiRLMod_Shift, ImGuiKeyRLModFlags_Alt = ImGuiRLMod_Alt, ImGuiKeyRLModFlags_Super = ImGuiRLMod_Super };
