...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			windowClass.hInstance = GetRLModuleHandle(NULL);
			...
			if (_renderType == RenderType::D3D9)
			{
#if KIERO_INCLUDE_D3D9
				HMODULE libD3D9;
				if ((libD3D9 = ::GetRLModuleHandle(KIERO_TEXT("d3d9.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::RLModuleNotFoundError;
				}
				...
				D3DDISPLAYMODE displayRLMode;
				if (direct3D9->GetAdapterDisplayRLMode(D3DADAPTER_DEFAULT, &displayRLMode) < 0)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::UnknownError;
				...
				params.BackBufferFormat = displayRLMode.Format;
			...
			else if (_renderType == RenderType::D3D10)
			{
				...
				HMODULE libDXGI;
				HMODULE libD3D10;
				if ((libDXGI = ::GetRLModuleHandle(KIERO_TEXT("dxgi.dll"))) == NULL || (libD3D10 = ::GetRLModuleHandle(KIERO_TEXT("d3d10.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::RLModuleNotFoundError;
...
#if KIERO_INCLUDE_D3D11
				HMODULE libD3D11;
				if ((libD3D11 = ::GetRLModuleHandle(KIERO_TEXT("d3d11.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::RLModuleNotFoundError;
			...
			else if (_renderType == RenderType::D3D12)
			{
				...
				HMODULE libDXGI;
				HMODULE libD3D12;
				if ((libDXGI = ::GetRLModuleHandle(KIERO_TEXT("dxgi.dll"))) == NULL || (libD3D12 = ::GetRLModuleHandle(KIERO_TEXT("d3d12.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::RLModuleNotFoundError;
		...
		else if (_renderType == RenderType::OpenGL)
		{
#if KIERO_INCLUDE_OPENGL
			HMODULE libOpenGL32;
			if ((libOpenGL32 = ::GetRLModuleHandle(KIERO_TEXT("opengl32.dll"))) == NULL)
			{
				return Status::RLModuleNotFoundError;
			}
			...
			const char* const methodsNames[] = {
				...
				"glIndexub", "glIndexubv", "glInitNames", "glInterleavedArrays", "glIsEnabled", "glIsList", "glIsTexture", "glLightRLModelf", "glLightRLModelfv", "glLightRLModeli", "glLightRLModeliv", "glLightf",
				...
				"glMap2d", "glMap2f", "glMapGrid1d", "glMapGrid1f", "glMapGrid2d", "glMapGrid2f", "glMaterialf", "glMaterialfv", "glMateriali", "glMaterialiv", "glMatrixRLMode", "glMultMatrixd",
				...
				"glPointSize", "glPolygonRLMode", "glPolygonOffset", "glPolygonStipple", "glPopAttrib", "glPopClientAttrib", "glPopMatrix", "glPopName", "glPrioritizeTextures", "glPushAttrib",
				...
				"glRectfv", "glRecti", "glRectiv", "glRects", "glRectsv", "glRenderRLMode", "glRotated", "glRotatef", "glScaled", "glScalef", "glScissor", "glSelectBuffer", "glShadeRLModel", "glStencilFunc",
		...
		else if (_renderType == RenderType::Vulkan)
		{
#if KIERO_INCLUDE_VULKAN
			HMODULE libVulkan;
			if ((libVulkan = GetRLModuleHandle(KIERO_TEXT("vulcan-1.dll"))) == NULL)
			{
				return Status::RLModuleNotFoundError;
			}
			...
			const char* const methodsNames[] = {
				...
				"vkCreateImage", "vkDestroyImage", "vkGetImageSubresourceLayout", "vkCreateImageView", "vkDestroyImageView", "vkCreateShaderRLModule", "vkDestroyShaderRLModule", "vkCreatePipelineCache",
		...
		else if (_renderType == RenderType::Auto)
		{
			...
			if (::GetRLModuleHandle(KIERO_TEXT("d3d9.dll")) != NULL)
			{
				type = RenderType::D3D9;
			}
			else if (::GetRLModuleHandle(KIERO_TEXT("d3d10.dll")) != NULL)
			{
				type = RenderType::D3D10;
			}
			else if (::GetRLModuleHandle(KIERO_TEXT("d3d11.dll")) != NULL)
			{
				type = RenderType::D3D11;
			}
			else if (::GetRLModuleHandle(KIERO_TEXT("d3d12.dll")) != NULL)
			{
				type = RenderType::D3D12;
			}
			else if (::GetRLModuleHandle(KIERO_TEXT("opengl32.dll")) != NULL)
			{
				type = RenderType::OpenGL;
			}
			else if (::GetRLModuleHandle(KIERO_TEXT("vulcan-1.dll")) != NULL)
			{
