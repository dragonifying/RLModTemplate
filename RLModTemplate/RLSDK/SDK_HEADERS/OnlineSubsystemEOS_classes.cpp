...
// Function OnlineSubsystemEOS.OnlineSubsystemEOS.OpenPS4DisplayRLMode
// [0x00024001] (FUNC_Final | FUNC_NoExport | FUNC_OptionalParm | FUNC_Public | FUNC_AllFlags)
// Parameter Info:
// uint8_t                        LocalUserNum                   (CPF_Parm)
// EPS4DisplayRLMode                DisplayRLMode                    (CPF_Parm)
// TArray<class FString>          Targets                        (CPF_OptionalParm | CPF_Parm | CPF_NeedCtorLink)
// int32_t                        ServiceLabel                   (CPF_OptionalParm | CPF_Parm)

void UOnlineSubsystemEOS::OpenPS4DisplayRLMode(uint8_t LocalUserNum, EPS4DisplayRLMode DisplayRLMode, TArray<class FString> Targets, int32_t ServiceLabel)
{
	static UFunction* uFnOpenPS4DisplayRLMode = nullptr;

	if (!uFnOpenPS4DisplayRLMode)
	{
		uFnOpenPS4DisplayRLMode = UFunction::FindFunction("Function OnlineSubsystemEOS.OnlineSubsystemEOS.OpenPS4DisplayRLMode");
	}

	UOnlineSubsystemEOS_execOpenPS4DisplayRLMode_Params OpenPS4DisplayRLMode_Params;
	memset(&OpenPS4DisplayRLMode_Params, 0, sizeof(OpenPS4DisplayRLMode_Params));
	memcpy_s(&OpenPS4DisplayRLMode_Params.LocalUserNum, sizeof(OpenPS4DisplayRLMode_Params.LocalUserNum), &LocalUserNum, sizeof(LocalUserNum));
	memcpy_s(&OpenPS4DisplayRLMode_Params.DisplayRLMode, sizeof(OpenPS4DisplayRLMode_Params.DisplayRLMode), &DisplayRLMode, sizeof(DisplayRLMode));
	memcpy_s(&OpenPS4DisplayRLMode_Params.Targets, sizeof(OpenPS4DisplayRLMode_Params.Targets), &Targets, sizeof(Targets));
	memcpy_s(&OpenPS4DisplayRLMode_Params.ServiceLabel, sizeof(OpenPS4DisplayRLMode_Params.ServiceLabel), &ServiceLabel, sizeof(ServiceLabel));

	this->ProcessEvent(uFnOpenPS4DisplayRLMode, &OpenPS4DisplayRLMode_Params, nullptr);
};
...
// Function OnlineSubsystemEOS.OnlineVoiceInterfaceEOS.CheckAllPlayersAcceptedVoiceRLModerationAgreement
// [0x00020401] (FUNC_Final | FUNC_Native | FUNC_Public | FUNC_AllFlags)
// Parameter Info:
// bool                           ReturnValue                    (CPF_Parm | CPF_OutParm | CPF_ReturnParm)

bool UOnlineVoiceInterfaceEOS::CheckAllPlayersAcceptedVoiceRLModerationAgreement()
{
	static UFunction* uFnCheckAllPlayersAcceptedVoiceRLModerationAgreement = nullptr;

	if (!uFnCheckAllPlayersAcceptedVoiceRLModerationAgreement)
	{
		uFnCheckAllPlayersAcceptedVoiceRLModerationAgreement = UFunction::FindFunction("Function OnlineSubsystemEOS.OnlineVoiceInterfaceEOS.CheckAllPlayersAcceptedVoiceRLModerationAgreement");
	}

	UOnlineVoiceInterfaceEOS_execCheckAllPlayersAcceptedVoiceRLModerationAgreement_Params CheckAllPlayersAcceptedVoiceRLModerationAgreement_Params;
	memset(&CheckAllPlayersAcceptedVoiceRLModerationAgreement_Params, 0, sizeof(CheckAllPlayersAcceptedVoiceRLModerationAgreement_Params));

	this->ProcessEvent(uFnCheckAllPlayersAcceptedVoiceRLModerationAgreement, &CheckAllPlayersAcceptedVoiceRLModerationAgreement_Params, nullptr);

	return CheckAllPlayersAcceptedVoiceRLModerationAgreement_Params.ReturnValue;
};
