...
enum EPropertyFlags : std::uint64_t
{
	...
	CPF_EditConstArray = 0x0000000000000040ULL,	// Prevent adding/removing of items from dynamic a array in the editor.
	...
	CPF_EdFindable = 0x0000000008000000ULL,	// References are set by clicking on actors in the editor viewports.
	...
	CPF_EditInlineNotify = 0x0000000040000000ULL,	// EditInline, notify outer object on editor change.
	...
	CPF_AutoWeak = 0x0000002000000000ULL,	// Auto-weak reference handling.
	...
	CPF_UObjectWrapper = 0x0004000000000000ULL,	// Property was parsed as a wrapper class like TSubclassOf , FScriptInterface etc., rather than a USomething*.
...
public:
	FNameEntry() : Flags(0), Index(-1), Name(L'\0'), UnknownData00{} {}
	~FNameEntry() {}
...
public:
	...
	{
		int32_t len32 = static_cast<int32_t>(other ? (wcslen(other) + 1) : 0);
		ArrayCount = len32;
		ArrayMax = len32;
		ArrayData = (len32 > 0 ? other : nullptr);
		return *this;
	}
