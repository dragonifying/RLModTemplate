...
{
	return RLRLMod::Format::ToDecimal(ToHex(false)); // Standard method would be "uint32_t decimalValue = (R & 0xFF) << 16) | ((G & 0xFF) << 8) | (B & 0xFF);"
}

uint32_t Color::ToDecimalAlpha() const
{
	return RLRLMod::Format::ToDecimal(ToHexAlpha(false));
}

std::string Color::ToHex(bool bNotation) const
{
	std::string hexStr = (bNotation ? "#" : "");
	hexStr += RLRLMod::Format::ToHex(static_cast<uint64_t>(R), 2, "");
	hexStr += RLRLMod::Format::ToHex(static_cast<uint64_t>(G), 2, "");
	hexStr += RLRLMod::Format::ToHex(static_cast<uint64_t>(B), 2, "");
	return hexStr;
}
...
std::string Color::ToHexAlpha(bool bNotation) const
{
	...
	hexStr += RLRLMod::Format::ToHex(static_cast<uint64_t>(A), 2, "");
	return hexStr;
}
...
{
	return FromHex(RLRLMod::Format::ToHex(decimalColor, ((decimalColor > 0xFFFFFF) ? 8 : 6), ""));
}

Color& Color::FromHex(std::string hexColor)
{
	RLRLMod::Format::RemoveAllCharsInline(hexColor, '#');

	if (RLRLMod::Format::IsStringHexadecimal(hexColor))
	{
		...
		if (hexColor.length() == 8)
		{
			alpha = RLRLMod::Format::ToDecimal(hexColor.substr(6, 2)); // Optional, if an alpha value is provided.
		}
		...
		{
			R = static_cast<uint8_t>(RLRLMod::Format::ToDecimal(hexColor.substr(0, 2)));
		}

		if (hexColor.length() >= 4)
		{
			G = static_cast<uint8_t>(RLRLMod::Format::ToDecimal(hexColor.substr(2, 2)));
		}

		if (hexColor.length() >= 6)
		{
			B = static_cast<uint8_t>(RLRLMod::Format::ToDecimal(hexColor.substr(4, 2)));
		}
	}
