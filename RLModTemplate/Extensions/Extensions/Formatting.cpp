...
namespace RLRLMod::Format
{
    ...
    std::string RandomString(uint32_t flags, size_t length)
    {
        ...
        if (length > 0)
        {
            ...
            if (!characterSet.empty())
            {
                ...
                {
                    randomStr += characterSet[Math::RandomRange<size_t>(0, (characterSet.length() - 1))];
                }
            }
        }
    ...
    char RandomCharacter(uint32_t flags)
    {
        ...
        if (!characterSet.empty())
        {
            ...
            {
                return characterSet[Math::RandomRange<size_t>(0, (characterSet.length() - 1))];
            }
