...
public:
	...
	//added this for drawing
	float Dot(const VectorI& other) const;
...
inline void GetAxes(const FRotator& rot, FVector& forward, FVector& right, FVector& up)
{
	float pitch = rot.Pitch * (3.1415926f / 32768.f);
	float yaw = rot.Yaw * (3.1415926f / 32768.f);
	...
	right = FVector(-sy, cy, 0.f);
	up = FVector(-sp * cy, -sp * sy, cp);
}
