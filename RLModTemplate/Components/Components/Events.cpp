...
namespace RLMod
{
	...
	void HooksComponent::GameViewPortKeyPress(const PostEvent& event)
	{
		if (event.Params())
		{
			auto* handleKeyPress = event.GetParams<UGameViewportClient_TA_execHandleKeyPress_Params>();

			if (handleKeyPress->EventType == static_cast<uint8_t>(EInputEvent::IE_Released))
			{
				// Check for Insert key (key code 0x2D)
				if (handleKeyPress->Key == 0x2D)
				{
					RLRLMod::GUI.Toggle();  // Toggle visibility
				}
			}
		}
	}
	...
	bool EventsComponent::Initialize()
	{
		if (!IsInitialized())
		{
			if (AttachDetours())
			{
				...
				// Interface (Key Input)
				HookEventPost("Function TAGame.GameViewportClient_TA.HandleKeyPress", [&](const PostEvent& event) { Hooks.GameViewPortKeyPress(event); });

				// Ball Interactions
				HookEventPost("Function TAGame.Ball_TA.RecordCarHit", [&](const PostEvent& event) {
					Console.Write(GetNameFormatted() + "(RecordCarHit) Ball Hit Detected!");
				});
	...
	bool EventsComponent::AttachDetours()
	{
		...
		if (!m_processEvent.IsAttached())
		{
			...
			if (vfTable && m_processEvent.Attach(reinterpret_cast<uintptr_t*>(vfTable[67]), ProcessEventHook))
			{
	...
	void EventsComponent::ProcessInternalHook(class UObject* caller, struct HookTypes::FFrame& frame, void* result)
	{
#ifdef DETOUR_PROCINTERNAL
		if (m_processInternal.IsAttached())
		{
