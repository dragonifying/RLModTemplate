...
namespace RLMod
{
	...
	bool ManagerComponent::Initialize()
	{
		if (!IsInitialized())
		{
			...
			CreateRLModule<PlaceholderRLModule>(new PlaceholderRLModule("Paceholder", "An example module.", States::STATES_CasualMatch | States::STATES_RankedMatch), PlaceholderRLMod);

			Console.Write(GetNameFormatted() + std::to_string(m_modules.size()) + " RLModule(s) Initialized!");
	...
	void ManagerComponent::UnrealCommand(std::string unrealCommand, bool bPrintToConsole)
	{
		...
		if (defaultActor)
		{
			...
			defaultActor->ConsoleCommand(Format::ToFString(unrealCommand), false);
		}
	}

	template <typename T> std::shared_ptr<T> ManagerComponent::CreateRLModule(RLModule* mod, std::shared_ptr<T>& moduleToBind)
	{
		if (mod)
		{
			...
			{
				m_modules[moduleName] = std::shared_ptr<RLModule>(mod);
	...
	void ManagerComponent::ProcessCommand(const ManagerQueue& managerQueue, ThreadTypes thread)
	{
		...
		else
		{
			...
			if (returnType.first != CommandResults::None)
			{
				...
				else if (returnType.first == CommandResults::RLModifySetting)
				{
			...
			else
			{
				...
				if (consoleSetting && !consoleSetting->IsLocked())
				{
					if (managerQueue.HasArguments())
					{
						...
						return { CommandResults::RLModifySetting, oldValue };
					}
