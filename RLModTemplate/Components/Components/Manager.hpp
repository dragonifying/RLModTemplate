...
#include "../../RLModules/Includes.hpp"
...
namespace RLMod
{
	...
	{
	private:
		std::map<std::string, std::shared_ptr<RLModule>> m_modules;
	...
	public:
		std::shared_ptr<PlaceholderRLModule> PlaceholderRLMod;
	...
	public:
		template <typename T> std::shared_ptr<T> CreateRLModule(RLModule* mod, std::shared_ptr<T>& moduleToBind);
		template <typename T> std::shared_ptr<T> GetRLModule(const std::string& moduleName)
		{
